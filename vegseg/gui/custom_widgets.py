...
"""
Created on Feb 09 16:08:02 2022
...
class MenuBox(Frame):
    ...
    def __init__(self, master=None, image_path=None, background='#303030', highlight='#558de8', **kw):
        ...
        self.menu = Menu(self.menu_button, tearoff=False, bg=background,
                         fg='black', bd=2, activebackground=highlight,
                         activeforeground='#558de8')
...
class DisplayCanvas(Frame):
    ...
    def __init__(self, master, background='#ffffff', highlight='#558de8', **kw):
        Frame.__init__(self, master, **kw)
        self.x = self.y = 0
        ...
        self.sbarv = ttk.Scrollbar(self, orient=VERTICAL)
        self.sbarh = ttk.Scrollbar(self, orient=HORIZONTAL)
    ...
    def update_image(self, image):
        """ Update image in the canvas field
        :param image: the single image instance
        :type image: PIL image
        :return: effect the behavior of GUI, includes: ...
        """
        self.draw = True
        ...
        if self.image_obj is None:
            ...
            # self.canvas.itemconfig(self.image_obj)
        self.sbarv.config(command=self.canvas.yview)
        self.sbarh.config(command=self.canvas.xview)
        self.canvas.config(scrollregion=self.canvas.bbox("all"))
    ...
    def clear(self):
        # TODO: The clear function is not working (Consider when I want to remove the current image)
        print('Go to Canvas Clear !!!')
